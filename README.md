...
## Troubleshooting & Known Issues (Portenta H7)

### 1. Hard Faults during `AllocateTensors()` (SDRAM Issues)
The Portenta H7 uses an external SDRAM chip for the tensor arena. If your board crashes (flashes the red Mbed OS error LED) exactly when printing `Ejecutando AllocateTensors() [PELIGRO]...`, this indicates a hardware failure with the SDRAM chip on your specific board (e.g., a cold solder joint or defective silicon). 
- **Symptom:** Code compiles and runs `setup()`, but hangs/crashes at tensor allocation.
- **Verification:** Sometimes cooling the board temporarily fixes the issue due to thermal contraction closing microscopic cracks in the BGA solder.
- **Solution:** You must use a Portenta H7 with a fully functional 8MB SDRAM chip for this framework, as the models (>2MB) do not fit in the internal SRAM.

### 2. COM Port Disappears on Windows (WSL / `usbipd` conflict)
If the Portenta allows uploading firmware in Bootloader mode but fails to show a COM port for the Serial Monitor when the sketch runs:
- **Symptom:** The Arduino IDE shows the board during upload, but the COM port vanishes afterwards.
- **Cause:** If you use Windows Subsystem for Linux (WSL) and have `usbipd` installed, it might be forcibly capturing the Portenta's USB PID/VID (`2341:025b`) as `Shared (forced)`.
- **Solution:** Open PowerShell as Administrator and run `usbipd list`. If the device is marked as `Shared (forced)`, release it using `usbipd unbind -b <bus-id>` (e.g., `usbipd unbind -b 3-2`) to return control to Windows.
