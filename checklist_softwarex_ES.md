...
4. [x] **`results/pil/` vs `results/hil/` — RESUELTO vía renombrado del archivo.** El script se renombró `hil_benchmark.py` → `pil_benchmark.py` (junto con `hil_firmware.ino` → `pil_firmware.ino`) para alinear con la taxonomía MIL→SIL→PIL→HIL, y como parte del mismo cambio su salida se movió de `results/hil/HIL_Confusion_Matrix.png` a `results/pil/PIL_Confusion_Matrix.png` — elimina la colisión con el banco de cámara. Parche completo con `git mv` + diffs línea por línea en `parche_renombrado_pil_ES.md` (7 archivos, 34 líneas) — **pendiente de aplicar en el repo local y hacer commit.**
