...
- [x] **Escalera de fidelidad MIL→SIL→PIL identificada en tus scripts reales** → tres eslabones ya implementados: **MIL** = `test_model.py` (float PC), **SIL** = `test_tflite_model.py` (INT8 simulado en PC), **PIL** = `hil_benchmark.py` + firmware serie (procesador real, datos inyectados por serie). *(Ojo taxonómico: lo que llamabas "HIL" es en rigor **PIL** — el chip real ejecuta, pero los estímulos entran como datos digitales, sin sensor físico en el lazo.)* **El eslabón HIL completo se aborda en la Fase 1C (camino 2).**
...
> **Repo actual:** `github.com/F4bian1012/phlame-tinyml` (branch de trabajo `Scripts-Depurados`; `default_branch = main`). Renombrado y con 10 topics ✅. **Última verificación:** 32 blobs (+1: `checklist_softwarex_ES.md` ya versionado en el propio repo), estructura limpia, 0 refs rotas. **Renombrado `hil_firmware`/`hil_benchmark` → `pil_firmware`/`pil_benchmark` verificado byte a byte: aplicado con precisión total** (7 archivos, incluido el fix crítico `results/pil/PIL_Confusion_Matrix.png` que elimina la colisión con el banco de cámara).
...
- [x] **Instrucciones de reproducción de la medición HIL**: el Quickstart documenta `hil_benchmark.py` + `hil_firmware.ino` con el protocolo `#...@`. → **HECHO.** *(Se robustece al añadir la tabla de resultados.)*
...
- [x] **Justificar `AllOpsResolver` en README (Firmware/Embedded Details)** → **HECHO.** Se amplió de una razón (evitar fallos de `AllocateTensors()` por operadores faltantes) a dos: también mantiene el firmware **agnóstico de modelo** entre las 4 arquitecturas soportadas (MobileNetV2/V3, ResNet18, SqueezeNet) sin mantenimiento de resolver por modelo — cada arquitectura usa un set de operadores distinto (hard-swish/squeeze-excite en V3, `Add` residual en ResNet18, `Concatenation` en los módulos fire de SqueezeNet). Se documentó también el trade-off honesto: binario más grande (todos los kernels enlazados) a cambio de esa flexibilidad — aceptable dados los 2MB de flash del Portenta H7. Útil si un revisor de perfil embebido pregunta por qué no `MicroMutableOpResolver`.
...
│   ├── hil_benchmark.py           # PIL  (chip real, img por serie)    ✅
...
4. [x] **`results/pil/` vs `results/hil/` — RESUELTO y VERIFICADO EN EL REPO.** El script se renombró `hil_benchmark.py` → `pil_benchmark.py` (junto con `hil_firmware.ino` → `pil_firmware.ino`), y su salida se movió de `results/hil/HIL_Confusion_Matrix.png` a `results/pil/PIL_Confusion_Matrix.png` — elimina la colisión con el banco de cámara. **Aplicado y confirmado en el árbol real del repo**: los 7 archivos del parche (`parche_renombrado_pil_ES.md`) coinciden exactamente con lo subido, incluidas las 7 referencias cruzadas en `hil_camera_benchmark.py` y el comentario en `hil_camera_firmware.ino`.
...
7. 🟡 **`.gitignore` de frames capturados — CONFIRMADO PENDIENTE.** `hil_camera_benchmark.py` usa `data/hil_frames/` como default de `--frames-dir` (activado por `--dump-frames`). Verificado en el `.gitignore` real del repo: solo ignora `data/raw/*`, `data/processed/*`, `data/augmented/*` — **`data/hil_frames/*` NO está listado.** Si corres el banco con `--dump-frames` antes de añadir esta línea, los frames de validación cruzada pueden colarse en un commit. Añadir:
   ```
   data/hil_frames/*
   !data/hil_frames/.gitkeep
   ```
